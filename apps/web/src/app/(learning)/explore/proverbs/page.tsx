      ...
      <main className="pt-24 pb-32">
        <PageContainer size="archive">
          ...
          <header className="mb-20 space-y-8 max-w-4xl">
            ...
            <p className="text-xl font-serif text-brand-earth/80 italic leading-relaxed border-l-2 border-brand-gold/40 pl-8">
          ...
          <div className="flex flex-col md:flex-row gap-6 mb-16">
            <div className="flex-1 relative group">
              <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-primary/40 group-focus-within:text-brand-gold transition-colors" />
              <input 
                type="text"
                placeholder="Search proverbs or meanings..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full bg-white dark:bg-zinc-900 border border-brand-indigo/10 rounded-3xl py-5 pl-16 pr-8 text-lg font-serif italic focus:outline-none focus:ring-4 focus:ring-brand-gold/10 focus:border-brand-gold transition-all shadow-sm"
              />
            </div>
            <button className="px-8 py-5 bg-white dark:bg-zinc-900 border border-brand-indigo/10 rounded-3xl text-[10px] font-black uppercase tracking-widest text-brand-primary hover:bg-brand-indigo/5 transition-all flex items-center gap-3">
          ...
          {loading ? (
            <div className="py-32 text-center animate-pulse">
               <div className="w-12 h-12 bg-brand-gold/20 rounded-full mx-auto mb-4" />
               <p className="text-[10px] font-black uppercase tracking-widest text-brand-indigo/40">Unearthing Wisdom...</p>
            </div>
          ) : filteredProverbs.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
               {filteredProverbs.map((proverb) => (
                 <div key={proverb.id} className="group relative bg-white dark:bg-zinc-900 rounded-[3rem] p-10 border border-brand-indigo/10 hover:border-brand-gold/30 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand-gold/5 overflow-hidden">
                    ...
                    <div className="space-y-8 relative z-10">
                       <div className="flex justify-between items-start">
                          <div className="flex items-center gap-2 px-3 py-1 bg-brand-indigo/10 rounded-lg border border-brand-indigo/10">
                             <Globe className="w-3 h-3 text-brand-indigo/60" />
                             <span className="text-[8px] font-black uppercase tracking-widest text-brand-indigo/60">
                                {proverb.variations[0]?.dialect.name || 'Standard Yorùbá'}
                             </span>
                          </div>
                          <div className="p-2 bg-brand-gold/20 text-brand-gold rounded-xl opacity-0 group-hover:opacity-100 transition-opacity">
                       ...
                       <div className="space-y-4">
                          ...
                          <p className="text-sm text-brand-earth/80 font-serif italic border-l-2 border-brand-indigo/10 pl-4">
                             &ldquo;{proverb.description || 'Literal meaning awaiting verification.'}&rdquo;
                          </p>
                       </div>

                       <div className="pt-8 border-t border-brand-indigo/10 flex items-center justify-between">
                          <div className="flex items-center gap-4">
                             <div className="w-10 h-10 rounded-full bg-brand-indigo/[0.05] border border-brand-indigo/10 flex items-center justify-center text-brand-indigo/60">
                                <Info className="w-4 h-4" />
                             </div>
                             <div>
                                <p className="text-[8px] font-black uppercase tracking-widest text-brand-indigo/40">Cultural Notes</p>
