  ...
  const fetchProfile = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/users/me`, {
         // Headers now automatically handled by fetch wrapper if we use apiRequest, 
         // but since this is a direct fetch for now, we'll manually add it or switch to apiRequest.
         // Let's use the token from localStorage directly for this component.
         headers: {
