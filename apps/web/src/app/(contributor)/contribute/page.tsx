      ...
      <main className="pt-24 pb-32">
        <PageContainer size="archive">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-8">
            <div className="space-y-4">
              ...
              <p className="text-lg text-brand-earth/60 font-serif italic max-w-xl">
                ...
                Follow the 3-step journey to preserve a linguistic artifact.
              </p>
            </div>

            <Link 
              href="/profile" 
              className="group flex items-center gap-4 bg-white dark:bg-zinc-900 border border-brand-indigo/5 p-4 pr-6 rounded-2xl shadow-sm hover:shadow-md transition-all"
            >
              <div className="w-12 h-12 rounded-xl bg-brand-indigo/5 flex items-center justify-center text-brand-indigo group-hover:bg-brand-indigo group-hover:text-white transition-colors">
                <History className="w-5 h-5" />
              </div>
              <div>
                <p className="text-[8px] font-bold uppercase tracking-widest text-text-secondary/40">Your Impact</p>
                <p className="text-xs font-bold text-brand-primary">View Contribution History</p>
              </div>
            </Link>
          </div>
          ...
          <section className="relative">
             <div className="absolute inset-0 bg-brand-indigo/[0.01] rounded-[3rem] -m-4 pointer-events-none border border-brand-indigo/[0.05]" />
             <ContributionStepper />
          </section>

          {/* Protocol Notice */}
          <div className="mt-24 max-w-2xl mx-auto p-8 rounded-[2rem] border border-dashed border-brand-indigo/10 text-center space-y-4">
