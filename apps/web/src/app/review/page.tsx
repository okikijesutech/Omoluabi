...
export default function ReviewPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [pendingRes, dialectsRes] = await Promise.all([
        fetch(`${apiBase}/reviews/pending`, { headers: { 'x-reviewer-test': 'true' } }),
  ...
  const handleReview = async (contributionId: string, approved: boolean) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/reviews/submit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-reviewer-test': 'true'
        },
        body: JSON.stringify({
        ...
        <PageContainer size="lg" className="pt-24 space-y-12">
          ...
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="bg-[#D8CFC7]/10 p-6 rounded-3xl border border-text-primary/5 flex gap-12 backdrop-blur-sm">
                ...
                <div className="text-center">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-text-secondary/40">Simulation</p>
                    <p className="text-[10px] font-bold text-brand-accent uppercase bg-brand-accent/10 px-2 py-1 rounded">Reviewer</p>
                </div>
            </div>
          </header>
